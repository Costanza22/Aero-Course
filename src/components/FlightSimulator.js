...
function stepSimulation(aircraft, controls) {
  const newSpeed = Math.max(0, aircraft.speed + (controls.throttle - 0.5) * 0.5);
  const newX = aircraft.x + Math.cos(aircraft.heading * Math.PI / 180) * newSpeed;
  const newY = aircraft.y - Math.sin(aircraft.heading * Math.PI / 180) * newSpeed;
  const newAltitude = Math.max(
    0,
    aircraft.altitude + Math.sin(aircraft.pitch * Math.PI / 180) * newSpeed * 10
  );
  let newHeading = (aircraft.heading + controls.rudder * 2) % 360;
  if (newHeading < 0) newHeading += 360;
  const newPitch = Math.max(-30, Math.min(30, aircraft.pitch + controls.elevator * 2));
  const newRoll = Math.max(-45, Math.min(45, aircraft.roll + controls.aileron * 3));
  ...
  const instruments = {
    airspeed: Math.round(newSpeed * 100),
    altitude: Math.round(newAltitude),
    heading: Math.round(newHeading),
    verticalSpeed: Math.round(Math.sin(newPitch * Math.PI / 180) * newSpeed * 100),
  };

  return { aircraft: nextAircraft, instruments };
}
...
function drawInstruments(ctx, instruments, controls, aircraft) {
...
const FlightSimulator = () => {
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      const { aircraft: nextAircraft, instruments: nextInstruments } = stepSimulation(prevAircraft, ctrl);
      ...
      drawBackground(ctx);
      drawAircraft(ctx, nextAircraft);
      drawInstruments(ctx, nextInstruments, ctrl, nextAircraft);
